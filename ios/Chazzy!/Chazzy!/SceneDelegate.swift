...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        let onboardingView = OnboardingScreen()
        
        let rootViewController = UIHostingController(rootView: messagesView)
        ...
        if let windowScene = scene as? UIWindowScene {
            ...
            DispatchQueue.main.async {
                ...
                rootViewController.present(onboardingViewController, animated: true) {
                    onboardingViewController.presentationController?.presentedView?.gestureRecognizers?.first?.isEnabled = false
                }
            }
        }
    }
